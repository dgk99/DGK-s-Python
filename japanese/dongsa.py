...
# 모든 동사를 모아보자
dongsa = ["する", "来(く)る", "食(た)る", "教(おし)える", "疲(つか)れる", "信(しん)じる", "晴(は)れる", "寝(ね)る", "見(み)る", "できる", "起(お)きる", "着(き)る","飲(の)む","持(も)つ","買(か)う","話(はな)す","死(し)ぬ", "会(あ)う","行(い)く","遊(あそ)ぶ","待(ま)つ","書(か)く","ある","乗(の)る","取(と)る","残(のこ)る","作(つく)る","始(はじ)まる","勉強(べんきょう)する","曲(まが)る","変(か)わる","頑張(がんば)る","参加(さんか)する","わかる","切(き)る","入(はい)る","要(い)る","知(し)る","走(はし)る","帰(かえ)る"]
