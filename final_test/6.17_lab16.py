...
# 리스트용 카운트 생성
count = 0
# 중복되지 않는 정수가 3개 나올 때 까지 반복해야 하니 while문에 조건문 사용
while count < 3:
    # 게임 시작 시 0~9 사이의 중복되지 않는 정수 3개를 생성
    word_random = random.randint(0, 9)
    # 중복값 검사
    for i in range(count):
        if word_random == com_list[i]:
            break
    else:
...
# 승리, 패배 조건을 달성할 때 까지 반복해야 하니 while문 사용
while True:
    # 플레이어 입력
